defmodule MetroCDMXChallenge do
  ...
  def metro_lines(xml_path) do
    ...
    # Return for example: ["1", "2", ...]
    stations_aliases = get_stations_aliases(xml)
    ...
    stations =
      Enum.map(stations_aliases, fn station_alias ->
  ...
  def mine_info_station(item) do
    %{
      name:
        List.to_string(item.name)
        |> String.downcase()
        |> String.normalize(:nfd)
        |> String.replace(~r/[^a-z\s]/u, ""),
      alias:
        List.to_string(item.line)
        |> String.split(".")
        |> Enum.at(0)
        |> String.split(" ")
        |> Enum.at(1),
  ...
  def mine_info_line(item) do
    %{
      ...
      coords:
        "#{item.coords}"
        |> String.split("\n")
        |> Enum.map(fn coord -> String.replace(coord, ~r"[a-z /]", "") end)
